...
class TransactionStorage(private val database: TransactionDatabase) : ITransactionStorage {
    ...
    override fun getLastTransactionBlockHeight(): Long? {
        return null//database.transactionDao().getLastTransaction()?.blockNumber
    }

    override fun getLastInternalTransactionBlockHeight(): Long? {
        return null//database.transactionDao().getLastInternalTransaction()?.blockNumber
    }

    override fun saveTransactions(transactions: List<EtherscanTransaction>) {
